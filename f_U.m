...
global kext kt kn Kong B posi Ndx nu

pos_x = Uc(Ndx+2:2*(Ndx+1))+posi(Ndx+2:2*(Ndx+1));
pos_z = Uc(1:Ndx+1)+posi(1:Ndx+1);

% part_int = [sign(posi(Ndx+3:2*(Ndx+1))-posi(Ndx+2))]';
...
% if(Uc(Ndx+1+floor(Ndx/2)+1,1)<Uc(Ndx+1+floor(Ndx/2)+2,1))
%     d12(floor(Ndx/2)+2,1) = -d12(floor(Ndx/2)+2,1);
%     d12sub(floor(Ndx/2)+1,1) = -d12sub(floor(Ndx/2)+1,1);
%     d12sup(floor(Ndx/2)+3,1) = -d12sup(floor(Ndx/2)+3,1);
% end
...
spKsh = (sparse(1,1:Ndx+1,v11,2*(Ndx+1),2*(Ndx+1))+...
...
spK = kext.*spKd+kt.*spKsh+kn.*Kong;
 
dU = -inv(B)*spK*Uc-inv(B)*Fc;
% dU(Ndx+1+floor(Ndx/2)+2,1) = dU(Ndx+2,1);
